...
def identify_intent(state: AgentState):
    #Identify the intent of the user and route to the appropriate agent
    
    response = llm.invoke(
        [SystemMessage(content="You are an AI assistant that identifies the intent of the user and routes to the appropriate agent Respond with exactly one word: 'portfolio' or 'calendar'"),
        HumanMessage(content=state["messages"][-1].content)]
    )

    user_input = state["messages"][-1].content

    return {"active_agent": response.content.lower().strip()}
